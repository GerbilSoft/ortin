...
// NDS Blowfish data.
static const uint8_t encr_data[] = {
...
// Encryption context.
// TODO: Make it an actual context later.
static uint32_t card_hash[0x412];
static int cardheader_devicetype = 0;		// TODO: DSi?
static uint32_t global3_x00, global3_x04;	// RTC value
static uint32_t global3_rand1;
static uint32_t global3_rand3;
static uint32_t arg2[3];
...
static void init1(uint32_t cardheader_gamecode)
{
	memcpy(card_hash, &encr_data, 4*(1024 + 18));
	arg2[0] = *(uint32_t *)&cardheader_gamecode;
	arg2[1] = (*(uint32_t *)&cardheader_gamecode) >> 1;
	arg2[2] = (*(uint32_t *)&cardheader_gamecode) << 1;
	init2(card_hash, arg2);
	init2(card_hash, arg2);
}

static void init0(uint32_t cardheader_gamecode)
{
	init1(cardheader_gamecode);
	encrypt(card_hash, (uint32_t*)&global3_x04, (uint32_t*)&global3_x00);
	global3_rand1 = global3_x00 ^ global3_x04;		// more RTC
	global3_rand3 = global3_x04 ^ 0x0380FEB2;
	encrypt(card_hash, (uint32_t*)&global3_rand3, (uint32_t*)&global3_rand1);
}
 ...
 * decrypt_arm9
 */
static int decrypt_arm9(uint32_t cardheader_gamecode, unsigned char *data)
{
 ...
 * encrypt_arm9
 */
static int encrypt_arm9(uint32_t cardheader_gamecode, unsigned char *data)
{
	uint32_t *p = (uint32_t*)data;
...
int encryptSecureArea(uint8_t *pRom)
{
	...
	encrypt_arm9(gamecode, &pRom[0x4000]);
	...
	// rounds table
	memcpy(&pRom[rounds_offsets], card_hash, 4*18);
	
	// S-boxes
	for (unsigned int i = 0; i < 4; i++) {
		memcpy(&pRom[sbox_offsets + (4*256*i)], &card_hash[18 + (i*256)], 4*256);
	}
